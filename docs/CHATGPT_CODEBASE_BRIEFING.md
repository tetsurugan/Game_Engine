...
**Scripts:** `npm run dev`, `build` (tsc + vite), **`build:gh-pages`** (production build with `VITE_BASE_PATH=/Game_Engine/` + `404.html` copy for GitHub Pages), `lint` (eslint), **`test`** / **`test:watch`** (Vitest, `src/engine/__tests__/**/*.test.ts`). **Public deploy:** push to **`main`** runs **`.github/workflows/deploy-github-pages.yml`**, which builds and pushes **`dist/`** to the **`gh-pages`** branch (**`peaceiris/actions-gh-pages`**). **GitHub Pages** must use **Deploy from branch → `gh-pages` / (root)** — not **`main`** (unbuilt `index.html` → blank site). Live: **https://tetsurugan.github.io/Game_Engine/** (Vite **`base`** + Router **`basename`** from **`import.meta.env.BASE_URL`**).
