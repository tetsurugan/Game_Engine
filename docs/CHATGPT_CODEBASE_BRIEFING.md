...
**Scripts:** `npm run dev`, `build` (tsc + vite), **`build:gh-pages`** (production build with `VITE_BASE_PATH=/Game_Engine/` + `404.html` copy for GitHub Pages), `lint` (eslint), **`test`** / **`test:watch`** (Vitest, `src/engine/__tests__/**/*.test.ts`). **Public deploy:** push to **`main`** runs **`.github/workflows/deploy-github-pages.yml`** → **https://tetsurugan.github.io/Game_Engine/** (Vite **`base`** + React Router **`basename`** from **`import.meta.env.BASE_URL`**).
