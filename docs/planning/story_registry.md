...
| `court_aftermath` | The Morning After Verdict | Modern / media fracture | `witness` | witness_interpretation | **shipped** | outcome_gated | Mark **`rumor_girl_proof_resolved`** + **any** of **`rumor_girl_witness_aftermath_invited`** \| **`rumor_girl_belief_gap_soft`** \| mark **`rumor_girl_fatal_aftermath_seeded`** (flags OR marks OR-group) | **Two-scene** proof (read draft → hallway): friend POV; **relational verdicts** (objective / awareness / private judgment / tendency); closings: shock-invited, fatal-mark edge, soft belief-gap — [`court_aftermath/story.ts`](../../src/stories/court_aftermath/story.ts) | personal → world |
